Here is a complete, beginner-friendly solution for a **Real-Time Speech-to-Speech Translator** using Azure AI Speech and Streamlit.

### 1. Project Summary
This app serves as a generic "Universal Translator." Users select an input language (what they speak) and an output language (what they want to hear). When they click the button, the app records their voice, translates the speech into text, displays both the original and translated text side-by-side, and then generates an audio file of the translation to listen to. It maintains a session history of all translations performed.

---

### 2. Azure Setup (Very Simple)

#### A. Azure Portal Setup
1.  Log in to the [Azure Portal](https://portal.azure.com/).
2.  Search for **"Speech services"** in the top search bar.
3.  Click **Create** > **Speech**.
4.  **Subscription:** Select yours.
5.  **Region:** Choose a region close to you (e.g., `eastus`). **Note this region.**
6.  **Name:** Enter a unique name (e.g., `speech-app-demo`).
7.  **Pricing Tier:** Select **Standard S0** (F0 Free tier works, but S0 is more reliable for translation).
8.  Click **Review + create** -> **Create**.

#### B. Get Credentials
1.  Go to your new Speech resource.
2.  On the left menu, click **Keys and Endpoint** (under Resource Management).
3.  Copy **Key 1**.
4.  Copy **Location/Region** (e.g., `eastus`, `westeurope`).

#### C. Add Credentials to Streamlit
Create a file named `.streamlit/secrets.toml` in your project folder (or paste into Streamlit Cloud secrets):

```toml
# .streamlit/secrets.toml
SPEECH_KEY = "your_pasted_key_here"
...
st.set_page_config(page_title="Azure Speech Translator", layout="wide")
...
# Azure Speech codes: https://learn.microsoft.com/en-us/azure/ai-services/speech-service/language-support
LANGUAGES = {
    "English (US)": "en-US",
    "Spanish": "es-ES",
    "French": "fr-FR",
...
def recognize_and_translate(source_lang_code, target_lang_code):
    ...
    # Note: 'recognize_once' is simpler for Streamlit than continuous loops
    ...
    if result.reason == speechsdk.ResultReason.TranslatedSpeech:
        ...
        # We create a new synthesizer to speak the *translated* text
...
def get_voice_for_language(lang_code):
    # Simple mapping for neural voices based on language code
    # This defaults to a female voice for the target language
    voices = {
...
3.  Run the application:
    ...
    ```
4.  The app will open in your browser. Ensure your microphone is allowed.

### Important Note on "Real-Time"
This app uses the **Single Utterance** mode (`recognize_once`). When you click the button, it listens until you pause for a breath or stop speaking, then processes the translation. This is the most stable method for a simple Python web app. Continuous streaming (infinite loop) requires advanced threading which makes the code complex and prone to freezing in Streamlit.
